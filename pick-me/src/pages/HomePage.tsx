  ...
  return (
    <div className="space-y-8">
      <section className="grid min-h-[calc(100vh-12rem)] items-center gap-10 lg:grid-cols-[0.95fr_1.05fr]">
        <div>
          <div className="mb-5 inline-flex rounded-full bg-grape/10 px-4 py-2 text-sm font-bold text-grape">
            Linkle katıl, birlikte oyna
          </div>
          <h1 className="max-w-3xl text-5xl font-black leading-[0.98] tracking-normal md:text-7xl">
            Arkadaş grubunu test et, sonuçları canlı gör.
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
            Hazır quiz seç, kendi sorularını oluştur, anonim modu aç ve oda linkini paylaş.
            Katılımcılar adla girer, oy verir, en sonda istatistikler görünür.
          </p>
        ...
        <div className="relative min-h-[520px]">
          <div className="rounded-[34px] border-[8px] border-[#191c25] bg-white p-5 shadow-2xl">
            ...
            <div className="panel-card">
              ...
              <button className="mt-5 h-12 w-full rounded-2xl bg-slate-100 font-extrabold text-slate-600">
                Oyumu kaydet
              </button>
            </div>
          </div>
          <div className="absolute bottom-0 right-0 w-64 rounded-[32px] border-[8px] border-[#191c25] bg-white p-5 shadow-2xl sm:w-72">
            ...
            {[
              ...
              <div className="mb-3 rounded-2xl bg-slate-50 p-3" key={name}>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Feature icon={<UsersRound />} title="Üyelikli oda" text="Quiz oluşturmak için kayıt/giriş var; katılımcılar linkle girebilir." />
        <Feature icon={<ShieldCheck />} title="Anonim ayar" text="Sonuçlarda oy veren isimleri gizle veya açık göster." />
        <Feature icon={<BarChart3 />} title="İstatistik" text="Her soru için oy sayısı, yüzde ve grafik görünümü." />
      </section>
      ...
      {rooms.length > 0 ? (
        <section className="panel-card">
          <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            <div>
              <h2 className="text-3xl font-black">Odaların</h2>
              <p className="text-slate-600">Bu hesapla oluşturduğun quizler.</p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {rooms.map((room) => (
              <Link className="rounded-2xl border border-slate-200 bg-white/80 p-4 transition hover:-translate-y-0.5" to={`/results/${room.id}`} key={room.id}>
...
function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <article className="panel-card">
      <div className="mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-grape/10 text-grape">{icon}</div>
